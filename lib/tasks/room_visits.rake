...
task :confirm_room_visits => :environment do
  ...
  room_visits.each do |room_visit|
    if room_visit.persona.cuenta_cimav
      VisitsMailer.confirm_room_visit(room_visit).deliver_later
      puts "[#{DateTime.now.to_s}][CONFIRMAR VISITA] Se notificará a #{room_visit.persona.nombre} sobre visita de #{room_visit.visit.institution} (id: #{room_visit.id})"
    end
  end
end
...
task :reminder_confirm_visit => :environment do
  ...
  visits.each do |visit|
    ...
    puts "[#{DateTime.now.to_s}][RECORDAR CONFIRMAR VISITA] Se recordará a #{visit.resp_name} confirmar visita de #{visit.institution} (id: #{visit.id})"
  end
end
...
task :cancel_unconfirmed_visits => :environment do
  ...
  visits.each do |visit|
    if visit.update(status: Visit::CANCELED)
      puts "[#{DateTime.now.to_s}][VISITA CANCELADA] Al no haber confirmación, se cancela visita de #{visit.institution} (id: #{visit.id})"
    end
  end
end
...
task :reminder_pending_requests => :environment do
  ...
  User.where(user_type: User::ASSISTANT).each do |assistant|
    ...
    if pending_requests > 0
      ...
      puts "[#{DateTime.now.to_s}][RECORDAR SOLICITUDES] Se recordará a #{assistant.name} sobre #{pending_requests} solicitudes pendientes en sus departamentos"
    end
  end
end